...
int main() {
  ...
  for (int j = 0; j <= temp; j++) {
    spraw = 0;
    lop = 0;
    for (int pot = i, h = 0; h <= i;
         h++, pot--) { // sprawdzanie dla jakich wartosci x wielomian ma wartosc
                       // 0
      spraw += wielo[h] * pow(dziel[j], pot);
    }
    if (spraw == 0) {
      for (int y = 0; y < j;
           y++) { // petla do zablokowania powtarzania wartosci x
        if (dziel[y] == dziel[j]) {
          lop++;
        }
      }
  ...
  int n = sizeof(wielo) /
          sizeof(wielo[0]); // sizeof - obliczenie ilosci zmiennych w tablicy
  wielomian(wielo, n, x);   // wykonanie funkcji
  return 0;
}
void wielomian(int wielo[], int n,
               int x) { // sprawdzenie czy do danego x na pewno wychodzi 0
  int wynik = wielo[0];
  for (int i = 1; i < n; i++) {
