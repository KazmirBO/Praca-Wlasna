...
# -*- coding: utf-8 -*-
#############################################################################
from PyQt5.QtCore import (QFile, Qt, QProcess)
...
from PyQt5.QtWidgets import (QApplication, QFileDialog, QMainWindow, QLineEdit,
                             ...
                             QLabel, QMessageBox, QToolButton, QComboBox)
...
#        self.dlCombo.activated[str].connect(self.setDownloadURL)
    ...
    def selectYTDL(self):
        fileName, _ = QFileDialog.getOpenFileName(
            self, "locate ytdlExec", "/usr/local/bin/ytdlExec",
            "exec Files (*)")
        if fileName:
...
border-radius: 2px;
background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #d3d3d3,
stop:1 #e9e9e9);
padding: 1px;
text-align: right;
margin-right: 4ex;
...
{
background: qlineargradient(x1: 0, y1: 0.5, x2: 1, y2: 0.5, stop: 0 blue,
stop: 1 white);
margin-right: 2px;
width: 8px;
}
